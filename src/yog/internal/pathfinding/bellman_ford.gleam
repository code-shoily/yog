...
pub type BellmanFordRawResult(e) {
  ShortestPathRaw(dist: e, nodes: List(NodeId))
  NegativeCycleRaw
  NoPathRaw
}
...
pub type ImplicitBellmanFordRawResult(cost) {
  FoundGoalRaw(cost)
  DetectedNegativeCycleRaw
  NoGoalRaw
}
...
) -> ImplicitBellmanFordRawResult(cost) {
...
fn do_implicit_bellman_ford(
  ...
  case queue.pop(q) {
    Error(Nil) -> {
      ...
      |> list.first()
      |> result.map(fn(entry) { FoundGoalRaw(entry.1) })
      |> result.unwrap(NoGoalRaw)
    }
...
fn do_implicit_bellman_ford_by(
  ...
  case queue.pop(q) {
    Error(Nil) -> {
      ...
      |> list.first()
      |> result.map(fn(entry) { FoundGoalRaw(entry.1.0) })
      |> result.unwrap(NoGoalRaw)
    }
    Ok(#(current, rest_queue)) -> {
