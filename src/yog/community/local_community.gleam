...
fn compute_k_out_in(
  ...
  set.fold(s, #(0.0, 0.0), fn(acc, node) {
    ...
    list.fold(model.successors(graph, node), #(k_in_acc, k_out_acc), fn(acc2, neighbor_rel) {
...
fn do_detect(
  ...
  case best_op {
    None -> s // Local maximum reached
    Some(Add(node, nk_in, nk_out)) -> {
      let new_s = set.insert(s, node)
      case iters >= opts.max_iterations {
        True -> new_s
        False -> do_detect(graph, new_s, nk_in, nk_out, final_cache, opts, iters + 1, weight_fn)
      }
    }
