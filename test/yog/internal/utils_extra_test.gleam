...
pub fn fisher_yates_basic_test() {
  let list = [1, 2, 3, 4, 5]
  let seed = 42

  let shuffled = utils.fisher_yates(list, seed)
...
pub fn fisher_yates_deterministic_test() {
  let list = [1, 2, 3, 4, 5]
  let seed = 42

  let shuffled1 = utils.fisher_yates(list, seed)
  let shuffled2 = utils.fisher_yates(list, seed)
...
pub fn fisher_yates_empty_test() {
  let result = utils.fisher_yates([], 123)
  result
  |> should.equal([])
}

pub fn fisher_yates_single_element_test() {
  let result = utils.fisher_yates([42], 999)
  result
  |> should.equal([42])
}

pub fn fisher_yates_two_elements_test() {
  let list = [1, 2]

  // With different seeds, should get one of the two permutations
  let shuffled = utils.fisher_yates(list, 42)
...
pub fn fisher_yates_preserves_elements_test() {
  let list = ["a", "b", "c", "d", "e"]
  let shuffled = utils.fisher_yates(list, 42)
...
pub fn fisher_yates_different_seeds_test() {
  let list = [1, 2, 3, 4, 5]
  let seed1 = 42
  let seed2 = 43

  let shuffled1 = utils.fisher_yates(list, seed1)
  let shuffled2 = utils.fisher_yates(list, seed2)
...
pub fn fisher_yates_multiple_shuffles_test() {
  let list = [1, 2, 3, 4]

  let shuffled1 = utils.fisher_yates(list, 42)
  let shuffled2 = utils.fisher_yates(shuffled1, 99)
