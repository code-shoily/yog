...
pub fn main() {
  ...
  let graph =
    ...
    |> yog.add_node(10, Nil)
  // Cluster A
  let assert Ok(graph) = yog.add_edge(graph, 1, 2, 10)
  let assert Ok(graph) = yog.add_edge(graph, 2, 3, 10)
  let assert Ok(graph) = yog.add_edge(graph, 3, 4, 10)
  let assert Ok(graph) = yog.add_edge(graph, 4, 5, 10)
  let assert Ok(graph) = yog.add_edge(graph, 5, 1, 10)
  // Cluster B
  let assert Ok(graph) = yog.add_edge(graph, 6, 7, 10)
  let assert Ok(graph) = yog.add_edge(graph, 7, 8, 10)
  let assert Ok(graph) = yog.add_edge(graph, 8, 9, 10)
  let assert Ok(graph) = yog.add_edge(graph, 9, 10, 10)
  let assert Ok(graph) = yog.add_edge(graph, 10, 6, 10)
  // The bridge (the minimum cut)
  let assert Ok(graph) = yog.add_edge(graph, 1, 6, 1)
